...
public class OpenAiService(IOpenAIService service, GatewayClient client) : IChatBot, IImageGen
{
    ...
    private ChatMessage DefaultSystemMessage =>
        ChatMessage.FromSystem(
            $"You are a discord bot called {client.Cache.User?.GetDisplayName()}. You are to assist with any requests to the best of your abilities.");
    ...
    {
        var serverId = guild?.Id ?? user?.Id ?? 0;
        var userName = user?.GetDisplayName() ?? "Anonymous";
    ...
    private List<ChatMessage> Reduce(List<ChatMessage> messages, int maxTokens = MaxTokens)
    {
        ...
        foreach (var msg in messages.Reverse<ChatMessage>())
        {
            if (msg.Role == StaticValues.ChatMessageRoles.User)
            {
                ...
                if (curTokenCount + msgTokenCount > maxTokens) break;
        ...
        if (reduced.All(x => x.Role != StaticValues.ChatMessageRoles.System)) reduced.Insert(0, DefaultSystemMessage);
